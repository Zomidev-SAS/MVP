-- Saldos de inventario (productos) publicados desde el Excel del panel.
-- Distinto de movimientos_inventario / vista_inventario_actual (vehículos VIN).
...
create policy "autenticados leen inventario saldos"
  on public.inventario_saldos for select
  to authenticated
  using (true);
...
create policy "supervisor compras publican inventario saldos"
  on public.inventario_saldos for all
  to authenticated
  using (
    exists (
      select 1 from public.profiles p
      where p.id = auth.uid() and p.rol in ('supervisor', 'compras')
    )
  )
  with check (
    exists (
...
create policy "autenticados leen inventario saldos meta"
  on public.inventario_saldos_meta for select
  to authenticated
  using (true);
...
create policy "supervisor compras actualizan inventario saldos meta"
  on public.inventario_saldos_meta for all
  to authenticated
  using (
    exists (
      select 1 from public.profiles p
